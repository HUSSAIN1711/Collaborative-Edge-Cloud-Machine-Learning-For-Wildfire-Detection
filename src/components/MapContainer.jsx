...
function MapContainer() {
    ...
    const interval = setInterval(() => {
      try {
        setPathIndex((prevIndex) => {
          ...
          if (nextPosition && typeof nextPosition.lat === 'number' && typeof nextPosition.lng === 'number') {
      ...
      const checkProximity = () => {
        try {
          ...
          sensors.forEach(sensor => {
            try {
              if (!sensor.position || typeof sensor.position.lat !== 'number' || typeof sensor.position.lng !== 'number') {
          ...
          if (!isNearAnySensor) {
            ...
            if (activeSensor) {
              console.log('Drone moved away from all sensors, clearing active sensor');
          ...
          if (nearSensor) {
            console.log(`Drone is near sensor ${nearSensor.id}, checking weather data...`);
            
            // Check if we already have cached weather data for this sensor
            const cachedWeather = getWeatherData(nearSensor.id);
            console.log('Cached weather for sensor', nearSensor.id, ':', cachedWeather);
            
            if (!cachedWeather) {
              console.log('Fetching weather data for sensor', nearSensor.id);
              weatherService.fetchWeatherData(nearSensor.position.lat, nearSensor.position.lng)
                .then(weatherData => {
              ...
              console.log('Using cached weather data for sensor', nearSensor.id);
            }
          }
        } catch (error) {
  ...
  }, [dronePosition, sensors, setSelectedSensor, getWeatherData, setWeatherData, activeSensor]);
  ...
  return isLoaded ? (
    ...
    >
      ...
      {dronePosition && typeof dronePosition.lat === 'number' && typeof dronePosition.lng === 'number' && (
        <Marker
