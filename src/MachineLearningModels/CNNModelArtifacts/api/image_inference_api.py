...
def handle_uncaught(e):
    """Ensure every error returns JSON so the frontend can show the real message."""
...
# Script lives in src/MachineLearningModels/CNNModelArtifacts/api/; model in CNNModelArtifacts/models/
SCRIPT_DIR = Path(__file__).resolve().parent
CNN_ARTIFACTS_ROOT = SCRIPT_DIR.parent  # CNNModelArtifacts directory
MODEL_PATH = CNN_ARTIFACTS_ROOT / "models" / "wildfire_resnet18.pth"
...
def load_model():
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(
            ...
            "Ensure wildfire_resnet18.pth is in CNNModelArtifacts/models/."
        )
...
def predict():
    ...
    try:
        ...
        try:
            ...
            app.logger.warning("Image open failed: %s", img_err)
            return jsonify({"error": f"Invalid or unsupported image: {img_err}"}), 400
        ...
        app.logger.exception("Model or file not found")
...
def predict_from_url():
    ...
    try:
        ...
        if not img_bytes:
            return jsonify({"error": "Image URL returned empty body"}), 400
        try:
            img = Image.open(io.BytesIO(img_bytes)).convert("RGB")
        except Exception as img_err:
            app.logger.warning("Image open failed for URL: %s", img_err)
            return jsonify({"error": f"Invalid or unsupported image from URL: {img_err}"}), 400
        ...
        app.logger.exception("Predict-from-URL failed")
        return jsonify({"error": str(e)}), 500
